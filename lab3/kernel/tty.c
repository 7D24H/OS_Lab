...
PUBLIC void in_process(TTY* p_tty, u32 key)
{
        ...
        if (!(key & FLAG_EXT)) {	// 如果是可显示字符，显示该字符
			...
			case ESC:
				find_mode = 1;
				//disp_str("Enter Find Mode");
				break;
			case ENTER:				// 回车键分2种情况
				if(find_mode){		// 如果当前是查找模式，按下回车表示结束输入，查找结果
					showing_result = 1;
					//disp_str("Show the Result");
					//disp_int(counterInFindArr);
					search_keyword();
...
{
	char searchHead = findCharArr[0];					// searchHead是要查找的字符串的首个字符
	
	for(int i=0; i<=counterInInputArr; ){				// 从头开始，遍历输入字符数组
		if(inputCharArr[i] == searchHead){				// 如果某个字符和searchHead一样
			int match = 1;	
			int j = 0;				
