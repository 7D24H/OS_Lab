...
PUBLIC void init_screen(TTY* p_tty)
{
	...
	find_mode = 0;		//当前不是查找模式
	showing_result = 0;	//当前没有在显示查找结果
...
PUBLIC void clean_screen()
{
	...
	for(int i=0;i<SCREEN_SIZE;i++){   //SCREE_SIZE  80*25
		disp_str(" ");
	}
	disp_pos=0;
...
PUBLIC void out_char(CONSOLE* p_con, char ch)
{
    ...
    case '\t':
		if (p_con->cursor < p_con->original_addr + p_con->v_mem_limit - 4) {
			...
			p_con->cursor = ( p_con->cursor/8 + 1 ) * 8;	// 光标移至下一个可以整除8的位置
		}
		break;
	...
	case '\b':
		if (p_con->cursor > p_con->original_addr) {
			...
			if(!find_mode){
				...
				if(counterInFindArr==-1){	//查找模式下沒有東西可以繼續被刪除了 這個鍵相當於失效了
					break;
				}
 ...
 *======================================================================*/
PUBLIC void show_char_arr(CONSOLE* p_con)
{
	int cursor_pos_save = p_con->cursor;	// 保存当前光标的位置
	
	u8* p_vmem;
		
	if(showing_result){						// 如果正在显示查找结果，以特殊颜色标出查找关键字
	
		// 重新显示被查找字符数组中的字符
		int i = 0;
		for(i=0; i<=counterInInputArr; i++){
			p_con->cursor = inputCharPositionArr[i];// 光标移至该字符记录的位置
			p_vmem = (u8*)(V_MEM_BASE + p_con->cursor * 2);
			...
			if(ch == '\n' || ch == '\t'){	// 如果输入字符数组中保存的是回车符或制表符
				...
				*p_vmem++ = ch;
				if(matchArr[i]){
					*p_vmem++ = LIGHT_BLUE_CHAR_COLOR;	// 如果命中，就以查找字符的颜色显示
				}
				else{
					*p_vmem++ = DEFAULT_CHAR_COLOR; 	// 没有命中就以默认颜色显示
				}
			}
		}
		...
		for(i=0; i<=counterInFindArr; i++){
			p_con->cursor = findCharPositionArr[i];// 光标移至该字符记录的位置
			p_vmem = (u8*)(V_MEM_BASE + p_con->cursor * 2);
			
			char ch = findCharArr[i];
			if(ch == '\t'){				// 如果查找字符数组中保存的是制表符（不需要考虑回车符的情况）
				*p_vmem++ = ' ';			// 以默认颜色显示空白符，反正光标已经移动到了该在的位置……
				*p_vmem++ = DEFAULT_CHAR_COLOR;
			}
			else{							// 如果查找字符数组中保存的是可显示字符
				*p_vmem++ = ch;
				*p_vmem++ = LIGHT_BLUE_CHAR_COLOR;	// 以查找字符的颜色显示
			}
		}
		
		// 重新显示光标的位置
		p_con->cursor = cursor_pos_save;		// 光标所在位置就是开头保存的值
		*p_vmem++ = ' ';
		*p_vmem++ = DEFAULT_CHAR_COLOR; 		// 以默认颜色显示光标
	}
		...
		for(i=0; i<=counterInInputArr; i++){
			...
			*p_vmem++ = ch;
			*p_vmem++ = DEFAULT_CHAR_COLOR; // 以默认颜色显示	
		}
		
		// 进入查找模式前输入的字符已经显示完毕，还要显示进入查找模式前光标的最后所在位置
		p_con->cursor = findCharPositionArr[0];		// 光标最后所在位置就是查找字符位置数组的第一个元素的值
		*p_vmem++ = ' ';
		*p_vmem++ = DEFAULT_CHAR_COLOR; 			// 以默认颜色显示光标
	}
